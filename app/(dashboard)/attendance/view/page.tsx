...
function ViewAttendancePageContent() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [recordsRes, summaryRes] = await Promise.all([
        authenticatedFetch(`/attendance/records?${params.toString()}`),
        authenticatedFetch(`/attendance/summary?${params.toString()}`),
      ]);
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await authenticatedFetch(`/attendance/upload`, {
        method: 'POST',
        body: formData,
      });
                    ...
                    <tr key={record.id} className="hover:bg-gray-50/50 dark:hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-xs ${record.employee.employeeType === 'ON_ROLL' ? 'bg-blue-500' : 'bg-amber-500'
                            }`}>
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-sm font-mono font-semibold ${record.overtimeHours > 0 ? 'text-green-600' : 'text-gray-400'
                          }`}>
                  ...
                  {summary.employeeStats.map((stat) => {
                    ...
                    return (
                      <tr key={stat.employee.id} className="hover:bg-gray-50/50 dark:hover:bg-slate-800/50 transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm ${stat.employee.employeeType === 'ON_ROLL' ? 'bg-blue-500' : 'bg-amber-500'
                              }`}>
