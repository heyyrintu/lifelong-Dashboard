...
export default function LoginPage() {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false); // Double-click protection
  const { user, loading: authLoading, login, register, sendEmailOTP, verifyEmailOTP, sendPhoneOTP, verifyPhoneOTP, loginWithGoogle, loginWithMicrosoft } = useAuth();
  ...
  // Show loading screen while checking auth status or if user is already logged in (redirecting)
  if (authLoading || (user && !hasRedirected.current)) {
    return (
                ...
                <div className="relative z-10">
                  ...
                  <div className="mb-4 md:mb-6 flex gap-2 p-1 rounded-full bg-white/95 dark:bg-slate-900/90 border border-gray-200/70 dark:border-slate-700/80 shadow-sm">
                    <button
                      ...
                      className={`flex-1 py-2.5 px-3 text-sm font-medium rounded-full transition-all duration-200 ${authMethod === 'email-password'
                    ...
                    <button
                      ...
                      className={`flex-1 py-2.5 px-3 text-sm font-medium rounded-full transition-all duration-200 ${authMethod === 'email-otp'
                        ? 'bg-brandRed text-white shadow-md ring-1 ring-brandRed/40'
                        : 'text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50/80 dark:hover:bg-slate-800/80'
                        }`}
                    >
                    ...
                    <button
                      ...
                      className={`flex-1 py-2.5 px-3 text-sm font-medium rounded-full transition-all duration-200 ${authMethod === 'phone-otp'
                        ? 'bg-brandRed text-white shadow-md ring-1 ring-brandRed/40'
                        : 'text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50/80 dark:hover:bg-slate-800/80'
                        }`}
                    >
